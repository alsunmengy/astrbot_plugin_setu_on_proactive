...
- 支持**延迟执行**：主动消息**最后一段发出后**延迟 N 秒再执行（`delay_before_execute`），多条指令之间可设间隔（`command_interval`）
- **自动避开分段**：主动消息插件分段/TTS 发送时每一段都会触发钩子，插件每次触发都会重置延迟任务——**只有最后一段触发的任务能存活**，指令（图）永远在全部文本段落发出之后出现，不会插在分段中间；一轮主动消息也天然只投递一次，无需手动去重
- 只联动单人私聊，群聊/频道不触发
...
| `delay_before_execute` | int | `0` | 主动消息**最后一段发出后**延迟 N 秒再投递指令。分段发送时每段触发都会重置计时，图永远在文本全部发出后出现 |
| `command_interval` | int | `0` | 多条指令之间的执行间隔（秒），避免瞬间连发被限流 |
| `dedup_window_seconds` | int | `60` | （v1.4.0 起由任务重置机制取代，保留兼容）无需手动配置 |

示例：配置 `commands = ["/setu 3 爱弥斯", "/签到"]`、`delay_before_execute = 10`、`command_interval = 3`——主动消息**全部分段发出 10 秒后**先执行 `/setu 3 爱弥斯`（图出现在文本之后），再隔 3 秒执行 `/签到`。
