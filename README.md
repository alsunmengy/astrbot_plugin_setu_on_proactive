...
- 支持**延迟执行**：主动消息触发后延迟 N 秒再执行（`delay_before_execute`），多条指令之间可设间隔（`command_interval`）
- 同一会话去重窗口（默认 60 秒，自动覆盖延迟执行周期），防止主动消息分段/TTS 多段发送触发重复指令
- 只联动单人私聊，群聊/频道不触发
...
| `delay_before_execute` | int | `0` | 主动消息触发后延迟 N 秒再执行指令（钩子立即返回，延迟在后台执行，不阻塞主动消息发送） |
| `command_interval` | int | `0` | 多条指令之间的执行间隔（秒），避免瞬间连发被限流 |
| `dedup_window_seconds` | int | `60` | 同一会话去重窗口（秒），窗口内只投递一次；实际窗口自动覆盖延迟执行周期 |

示例：配置 `commands = ["/setu 3 爱弥斯", "/签到"]`、`delay_before_execute = 5`、`command_interval = 3`，主动消息发出后 5 秒开始执行，先发 `/setu 3 爱弥斯`，再隔 3 秒发 `/签到`。
